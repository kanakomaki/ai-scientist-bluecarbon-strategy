...
def combine_results(base_dir):
    ...
    output_dir = base_dir / f"experiments_outputs" / f"combined_{timestamp}"
    output_dir.mkdir(parents=True, exist_ok=True)
    print(f"------Combining results folder created: {output_dir}")

    combined_md_path = output_dir / "combined_summary.md"
    combined_json_path = output_dir / "combined_result_summary.json"
    combined_fig_dir = output_dir / "combined_figures"
    combined_fig_dir.mkdir(exist_ok=True)

    combined_md_lines = ["# Combined Experiment Summary\n\n"]
    combined_json = {}

    # experiments directory = predict_mangrove, typhoon_analysis, graph_strategy...
    experiments_parent = base_dir / "experiments_outputs"
    for exp_parent in experiments_parent.iterdir():
        if not exp_parent.is_dir():
            continue
        if exp_parent.name.startswith("combined_"):
            continue  # skip already combined directories

        exp_name = exp_parent.name

        # run_* subdirectories search
    ...
    print(f"------Results combined into: {output_dir}")
...
if __name__ == "__main__":
    ...
    if args.base_dir:
        ...
        base_dir = Path(__file__).resolve().parent.parent  # running from templates/tyhoon_analysis
