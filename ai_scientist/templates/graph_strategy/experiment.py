...
def ask_sme_connections():
    ...
    connects_to = []
    if input("2. Do you intend to collaborate with NGOs? (yes/no): ").strip().lower().startswith("y"):
        connects_to.append("NGO")
    if input("3. Will you seek funding from GCF or UNEP? (yes/no): ").strip().lower().startswith("y"):
        connects_to.extend(["Green Climate Fund (GCF)", "UNEP"])
    if input("4. Will you need verification partners? (yes/no): ").strip().lower().startswith("y"):
...
def inject_sme_node(G: nx.Graph, support: str, connects_to: list):
    ...
    if support.startswith("y"):
        if "Ministry of the Environment of Japan" in G:
            G.add_edge("SME", "Ministry of the Environment of Japan", relation="supported by")
    for node in connects_to:
        if node in G:
            G.add_edge("SME", node, relation="wishes to connect to")
    print(connects_to)
    print("SME's edges:")
    for u, v, attrs in G.edges("SME", data=True):
        print(f"{u} → {v}: {attrs}")
...
def save_graph_image(G, output_path):
    ...
    pos = nx.spring_layout(G, seed=42)  # or use graphviz_layout if available
...
def save_focused_graph_image(G, output_path, focus_node="SME", radius=5):

    # SME focued graph
    subG = nx.ego_graph(G, focus_node, radius=radius)
    pos = nx.spring_layout(subG, k=1.5, iterations=200, seed=42)

    # Highlight the focus node
    node_colors = ["red" if n == focus_node else "skyblue" for n in subG.nodes()]
    node_sizes = [800 if n == focus_node else 400 for n in subG.nodes()]
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_path, "focused_graph.png"))
    plt.close()
...
def run_experiment(output_dir, config):
    ...
    # search for nodes with specific Carbon Credit Schemes
    ...
    output_path = os.path.join(output_dir, "result_summary.md")
    ...
    # save_graph_image(G, output_dir)
    save_focused_graph_image(G, output_dir)
...
def main():
    ...
    output_path=f"experiments_outputs/graph_strategy/run_{timenow:%Y%m%d_%H%M%S}/"
    os.makedirs(output_path, exist_ok=True)

    parser = argparse.ArgumentParser(description="AI Scientist: Social Strategy Recommendations with GraphDB")
    parser.add_argument("--output_dir", type=str, default=output_path, help="Directory to save output files")
    parser.add_argument("--graphml", type=str, default="ai_scientist/templates/graph_strategy/inputs/merged_project_graph.graphml", help="Input GraphML file")
