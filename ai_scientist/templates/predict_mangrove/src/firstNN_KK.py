...
def plotNVDIBand(input_data,output_path=None):
    plt.figure(figsize=(24,6))
    plt.imshow(input_data, cmap='RdYlGn', vmin=-1, vmax=1)
    plt.colorbar()
    if output_path:
        plt.savefig(output_path+"plotNVDIBand.png", bbox_inches='tight')
    #plt.show()

def plotMangroveBand(input_data,output_path=None):
    plt.figure(figsize=(16,6))
    plt.imshow(input_data, cmap=discrete_cmap, vmin=vmin, vmax=vmax)
    # plt.colorbar()
    if output_path:
        plt.savefig(output_path+"plotMangroveBand.png", bbox_inches='tight')
    #plt.show()

def plotDifference(labels_data, predicted_data,output_path=None):
    '''
    ...
    plt.colorbar()
    if output_path:
        plt.savefig(output_path+"plotDifference.png", bbox_inches='tight')
    #plt.show()
...
def makeROCPlot(y_actual, y_predicted_prob, output_path=None):
    ...
    plt.legend(loc='best')
    if output_path:
        plt.savefig(output_path+"ROCPlot.png", bbox_inches='tight')
    #plt.show()
...
def predictOnImage(model, image, output_path=None):
  ...
  if input_bands == ndvi_band: 
    print('\nNDVI band:')
    plotNVDIBand(features_new, output_path)

  # plot Mangrove band
  print('\nLabel mangroves from 2000 data:')
  plotMangroveBand(labels_new, output_path)
  ...
  makeROCPlot(labels_new_input, predicted_new_image_prob, output_path)
